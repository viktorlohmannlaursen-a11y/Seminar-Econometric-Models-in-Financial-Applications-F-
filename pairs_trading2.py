...
def process_pair_johansen(pair: tuple,
    ...
    for i in range(n_windows):
        ...
        if best_res.lr1[0] >= crit[0]:
            ...
            # Extract beta_hat from first Johansen eigenvector (normalised)
            evec = best_res.evec[:, 0]
            beta_hat = float(np.clip(-evec[1] / evec[0]
                                     if abs(evec[0]) > 1e-12 else 1.0,
                                     0.1, 10.0))
...
# ===========================================================================
# 5C. VANILLA FREE-BETA — OOS PROCESSING  (uses Johansen beta_hat)
# ===========================================================================

def process_pair_oos_vanilla_freebeta(pairs_df_filtered: pd.DataFrame,
                                      prices_oos: pd.DataFrame) -> list:
    """
    Vanilla pairs trading with the freely-estimated Johansen hedge ratio.
    Spread: z_t = ln(P_y) - beta_hat * ln(P_x)
    Position sizing: ±beta_hat units of X per 1 unit of Y.
    Everything else (thresholds, logic) identical to the beta=1 vanilla.
    """
    records = []
    ...
    for _, row in pairs_df_filtered.iterrows():
        sy = row["Stock_y"]
        sx = row["Stock_x"]
        beta = float(row.get("beta_hat", 1.0))
        ...
        for j in range(len(z)):
            zs = z_score.iloc[j]
            if not in_pos:
                if abs(zs) > VANILLA_ENTRY_ZSCORE:
                    trigger[j] = 1
                    in_pos = True
            else:
                if abs(zs) < VANILLA_EXIT_ZSCORE:
...
def run_oos_processing_vanilla_freebeta(pairs_df_johansen: pd.DataFrame,
                                        prices: pd.DataFrame) -> pd.DataFrame:
    """Parallel OOS runner for the free-beta vanilla strategy."""
    unique_end_dates = sorted(pairs_df_johansen["End_Date"].unique())
    print(f"\n[OOS-VANILLA-FB] Processing {len(unique_end_dates)} trading windows ...")

    n_jobs = max(1, multiprocessing.cpu_count() - 1)

    def _process_one_window(end_date):
        pf    = pairs_df_johansen[pairs_df_johansen["End_Date"] == end_date]
        p_oos = prices[prices.index >= end_date].head(STEP)
        if len(p_oos) < 5:
            return []
        return process_pair_oos_vanilla_freebeta(pf, p_oos)

    all_records = Parallel(n_jobs=n_jobs, verbose=0)(
        delayed(_process_one_window)(ed) for ed in unique_end_dates
    )
    ...
    if not oos_df.empty:
        oos_df["Date"] = pd.to_datetime(oos_df["Date"])
        path = os.path.join(OUTPUT_DIR, "oos_vanilla_freebeta_SP500.csv")
        oos_df.to_csv(path, index=False)
        print(f"[OOS-VANILLA-FB] Saved to {path}  ({len(oos_df):,} rows)")
    return oos_df


def calculate_pair_returns_vanilla_freebeta(pair_data: pd.DataFrame) -> np.ndarray:
    """
    Daily P&L for the free-beta vanilla strategy.
    Long 1 unit of Y, short beta_hat units of X.
    """
    n     = len(pair_data)
    rets  = np.full(n, np.nan)
    pos   = 0  # +1 long Y / short X, -1 short Y / long X, 0 flat

    for j in range(n):
        trig = pair_data["Trigger"].iloc[j]
        beta = float(pair_data["beta_hat"].iloc[j])

        if j == 0:
            if trig == 1:
                pos = int(np.sign(pair_data["Z_Score"].iloc[j]))
            continue

        py_prev = pair_data["Price_y"].iloc[j - 1]
        px_prev = pair_data["Price_x"].iloc[j - 1]
        py_now  = pair_data["Price_y"].iloc[j]
        px_now  = pair_data["Price_x"].iloc[j]

        if pos != 0:
            ret_y = (py_now - py_prev) / py_prev
            ret_x = (px_now - px_prev) / px_prev
            rets[j] = pos * (ret_y - beta * ret_x) / (1.0 + beta)

        if trig == 0:
            pos = 0
        elif trig == 1 and pos == 0:
            pos = int(np.sign(pair_data["Z_Score"].iloc[j]))

    return rets


def aggregate_returns_vanilla_freebeta(oos_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate free-beta vanilla daily returns (equal-weighted)."""
    returns_dict = {}
    count_dict   = {}
    ...
    for (sy, sx), grp in oos_df.groupby(["Stock_y", "Stock_x"]):
        ...
        for w in range(n_windows):
            ...
            if batch["Trigger"].isna().all():
                continue
            rets = calculate_pair_returns_vanilla_freebeta(batch)
            for date, ret in zip(batch.index, rets):
                if np.isnan(ret):
                    continue
                if date in returns_dict:
...
def _plot_vanilla_beta_comparison(combined_beta1: pd.DataFrame,
                                  combined_freebeta: pd.DataFrame,
                                  save_dir: str = OUTPUT_DIR):
    """Side-by-side cumulative return and drawdown comparison."""
    common = combined_beta1.index.intersection(combined_freebeta.index)
    if len(common) < 10:
        return

    cum1  = (1 + combined_beta1.loc[common,  "Daily_Return"]).cumprod()
    cumfb = (1 + combined_freebeta.loc[common, "Daily_Return"]).cumprod()
    ...
    axes[0].plot(cum1.index,  cum1.values,  color="steelblue", lw=1.3, label="β=1")
    axes[0].plot(cumfb.index, cumfb.values, color="#e67e22",   lw=1.3, label="Free-β")
    axes[0].axhline(1, color="gray", lw=0.8, ls="--")
    axes[0].set_title("Cumulative Return: Vanilla β=1 vs Free-β", fontweight="bold")
    axes[0].set_ylabel("Cumulative Return (×)")
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)

    axes[1].fill_between(_dd(cum1).index,  _dd(cum1).values,  0,
                         alpha=0.4, color="steelblue", label="β=1")
    axes[1].fill_between(_dd(cumfb).index, _dd(cumfb).values, 0,
                         alpha=0.4, color="#e67e22",   label="Free-β")
    axes[1].set_title("Drawdown: Vanilla β=1 vs Free-β", fontweight="bold")
    ...
    fig.autofmt_xdate()
    plt.tight_layout()
    path = os.path.join(save_dir, "vanilla_beta_comparison_cumret.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[PLOT] Saved → {path}")

    path2 = os.path.join(save_dir, "vanilla_beta_comparison_dd.png")
    plt.savefig(path2, dpi=150, bbox_inches="tight")
    plt.close("all")


def _print_vanilla_beta_comparison_table(combined_beta1: pd.DataFrame,
                                         combined_freebeta: pd.DataFrame):
    """Print a side-by-side performance table for the two vanilla variants."""
    def _stats(df):
        r = df["Daily_Return"].dropna()
        ann_ret = r.mean() * YEAR
        ann_vol = r.std() * np.sqrt(YEAR)
        sr      = ann_ret / ann_vol if ann_vol > 1e-12 else np.nan
        cum     = (1 + r).cumprod()
        roll    = cum.cummax()
        mdd     = ((cum - roll) / roll).min()
        return ann_ret, ann_vol, sr, mdd

    r1, v1, s1, d1 = _stats(combined_beta1)
    rf, vf, sf, df_ = _stats(combined_freebeta)

    print(f"\n{'='*65}")
    print("  VANILLA β=1 vs FREE-β PERFORMANCE COMPARISON")
    print(f"{'='*65}")
    print(f"  {'Metric':<40} {'β=1':>10} {'Free-β':>10}")
    print(f"  {'-'*60}")
    print(f"  {'Ann. Return':<40} {r1*100:>9.2f}% {rf*100:>9.2f}%")
    print(f"  {'Ann. Volatility':<40} {v1*100:>9.2f}% {vf*100:>9.2f}%")
    print(f"  {'Sharpe Ratio':<40} {s1:>10.2f} {sf:>10.2f}")
    print(f"  {'Max Drawdown':<40} {d1*100:>9.1f}% {df_*100:>9.1f}%")
    print(f"{'='*65}")
...
# ---------------------------------------------------------------------------
#  FAMA-FRENCH 5-FACTOR + MOMENTUM (FF5+MOM) REGRESSION
# ---------------------------------------------------------------------------

def run_fama_french_momentum(
    combined_df: pd.DataFrame,
    rates: pd.Series,
    save_dir: str = OUTPUT_DIR,
) -> dict:
    """
    Regress the options-strategy daily excess returns on the Fama-French
    5 factors plus the Momentum (UMD) factor.

    Model: R_pt - R_ft = α + β_MKT·MKT + β_SMB·SMB + β_HML·HML
                           + β_RMW·RMW + β_CMA·CMA + β_MOM·MOM + ε

    Standard errors are Newey-West HAC.
    Saves: ff5mom_regression_table.csv, ff5mom_factor_loadings.png,
           ff5mom_cumulative_alpha.png, ff5mom_rolling_mom_beta.png
    """
    print("\n" + "="*60)
    ...
    if not PANDAS_DATAREADER_OK:
        print("[FF5+MOM] pandas_datareader not available — skipping.")
        return {}
    ...
    try:
        ...
        ff5 = ff5 / 100.0
        print(f"[FF5+MOM] FF5 factors downloaded: {len(ff5):,} observations.")
    except Exception as e:
        print(f"[FF5+MOM] Failed to download FF5 factors: {e}")
        return {}
    ...
    try:
        ...
        mom = mom / 100.0
        mom.columns = ["MOM"]
        print(f"[FF5+MOM] MOM factor downloaded: {len(mom):,} observations.")
    except Exception as e:
        print(f"[FF5+MOM] Failed to download MOM factor: {e}")
        mom = pd.DataFrame()

    # --- Build factor DataFrame ----------------------------------------------
    factors = ff5.rename(columns={"Mkt-RF": "MKT"})
    if not mom.empty:
        factors = factors.join(mom, how="inner")
        factor_cols = ["MKT", "SMB", "HML", "RMW", "CMA", "MOM"]
    else:
        factor_cols = ["MKT", "SMB", "HML", "RMW", "CMA"]
        print("[FF5+MOM] Running FF5 only (MOM unavailable).")

    # --- Strategy excess returns ---------------------------------------------
    strat_ret = combined_df["Daily_Return"]
    rf_ff     = factors["RF"] if "RF" in factors.columns else pd.Series(0, index=factors.index)

    common = strat_ret.index.intersection(factors.index)
    if len(common) < 100:
        print(f"[FF5+MOM] Too few overlapping observations ({len(common)}) — skipping.")
        return {}

    y = (strat_ret.loc[common] - rf_ff.loc[common]).rename("Excess_Ret")
    X = factors.loc[common, factor_cols]
    print(f"[FF5+MOM] Regression sample: {common[0].date()} to {common[-1].date()} "
          f"({len(common):,} obs)")

    # --- OLS with Newey-West HAC SEs -----------------------------------------
    X_const = add_constant(X, has_constant="add")
    X_const.columns = ["const"] + list(factor_cols)
    nw_lags = int(4 * (len(y) / 100) ** (2.0 / 9.0))
    nw_res  = OLS(y, X_const).fit(
        cov_type="HAC", cov_kwds={"maxlags": nw_lags, "use_correction": True}
    )

    alpha     = nw_res.params["const"]
    t_alpha   = nw_res.tvalues["const"]
    p_alpha   = nw_res.pvalues["const"]
    alpha_ann = alpha * YEAR
    r2        = nw_res.rsquared
    r2_adj    = nw_res.rsquared_adj

    # --- Print results -------------------------------------------------------
    def _sig(p):
        return "***" if p < 0.01 else ("**" if p < 0.05 else ("*" if p < 0.10 else ""))

    print(f"\n  {'Factor':<10} {'Coef':>10} {'t-stat':>10} {'p-value':>10}  Sig")
    print(f"  {'-'*48}")
    for name in ["const"] + factor_cols:
        coef = nw_res.params[name]
        tv   = nw_res.tvalues[name]
        pv   = nw_res.pvalues[name]
        lbl  = "Alpha" if name == "const" else name
        print(f"  {lbl:<10} {coef:>10.6f} {tv:>10.3f} {pv:>10.4f}  {_sig(pv)}")
    print(f"  {'-'*48}")
    print(f"  R²         {r2:>10.4f}")
    print(f"  Adj. R²    {r2_adj:>10.4f}")
    print(f"  Alpha (ann){alpha_ann*100:>9.2f}%")
    print(f"  t(α)       {t_alpha:>10.3f}   p={p_alpha:.4f} {_sig(p_alpha)}")

    mom_coef = nw_res.params.get("MOM", np.nan)
    mom_t    = nw_res.tvalues.get("MOM", np.nan)
    mom_p    = nw_res.pvalues.get("MOM", np.nan)
    print(f"\n  Interpretation:")
    if not np.isnan(mom_coef):
        if abs(mom_t) > 1.96:
            direction = "positive (momentum-chasing)" if mom_coef > 0 else "negative (contrarian)"
            print(f"  → MOM loading is significant (t={mom_t:.2f}, {direction}).")
        else:
            print(f"  → MOM loading is NOT significant (t={mom_t:.2f}). "
                  "Returns are not momentum-driven.")
    if abs(t_alpha) > 1.96:
        print(f"  → Significant alpha of {alpha_ann*100:.2f}% p.a. — "
              "strategy earns factor-adjusted excess returns.")
    else:
        print(f"  → Alpha ({alpha_ann*100:.2f}% p.a.) is NOT significant.")

    # --- Save CSV ------------------------------------------------------------
    results_df = pd.DataFrame({
        "Factor"  : ["Alpha (daily)", "Alpha (annual)"] + factor_cols,
        "Coef"    : [alpha, alpha_ann] + [nw_res.params[f] for f in factor_cols],
        "t_stat"  : [t_alpha, t_alpha] + [nw_res.tvalues[f] for f in factor_cols],
        "p_value" : [p_alpha, p_alpha] + [nw_res.pvalues[f] for f in factor_cols],
        "Sig"     : [_sig(p_alpha), _sig(p_alpha)] +
                    [_sig(nw_res.pvalues[f]) for f in factor_cols],
    })
    results_df["R_squared"]     = r2
    results_df["R_squared_adj"] = r2_adj
    csv_path = os.path.join(save_dir, "ff5mom_regression_table.csv")
    results_df.to_csv(csv_path, index=False)
    print(f"\n[FF5+MOM] Regression table saved → {csv_path}")

    # --- Plot A: Factor loadings bar chart -----------------------------------
    plot_labels = ["α×252"] + factor_cols
    plot_vals   = [alpha_ann] + [nw_res.params[f] for f in factor_cols]
    plot_errs   = [nw_res.bse["const"] * 1.96 * YEAR] + \
                  [nw_res.bse[f] * 1.96 for f in factor_cols]
    colors = ["#2ecc71" if v >= 0 else "#e74c3c" for v in plot_vals]

    fig, ax = plt.subplots(figsize=(11, 5))
    bars = ax.bar(plot_labels, plot_vals, color=colors, edgecolor="white",
                  linewidth=0.6, zorder=3)
    ax.errorbar(plot_labels, plot_vals, yerr=plot_errs, fmt="none",
                color="black", capsize=4, lw=1.2, zorder=4)
    ax.axhline(0, color="black", lw=0.8)
    ax.set_title("FF5 + Momentum Factor Loadings (Newey-West 95% CI)",
                 fontweight="bold", fontsize=12)
    ax.set_ylabel("Coefficient (α annualised, β daily)")
    ...
    plt.tight_layout()
    path = os.path.join(save_dir, "ff5mom_factor_loadings.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[PLOT] Saved → {path}")

    # --- Plot B: Cumulative raw vs factor-adjusted returns -------------------
    fitted_factors = X.dot(nw_res.params[factor_cols])
    alpha_ret      = y - fitted_factors
    cum_raw   = (1 + y).cumprod()
    cum_alpha = (1 + alpha_ret).cumprod()

    fig, ax = plt.subplots(figsize=(12, 5))
    ax.plot(cum_raw.index,   cum_raw.values,   color="steelblue", lw=1.3,
            label="Strategy Excess Return")
    ax.plot(cum_alpha.index, cum_alpha.values, color="#e67e22",  lw=1.3,
            label="Factor-Adjusted Return (α only)")
    ax.axhline(1, color="gray", lw=0.8, ls="--")
    ax.set_title("Cumulative Returns: Raw Excess vs Factor-Adjusted",
                 fontweight="bold", fontsize=12)
    ax.set_ylabel("Cumulative Return (×)")
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ...
    fig.autofmt_xdate()
    plt.tight_layout()
    path = os.path.join(save_dir, "ff5mom_cumulative_alpha.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[PLOT] Saved → {path}")

    # --- Plot C: Rolling 1-year MOM beta -------------------------------------
    if "MOM" in factor_cols:
        roll_window   = YEAR
        rolling_mom   = []
        rolling_dates = []
        for i in range(roll_window, len(y)):
            y_roll = y.iloc[i - roll_window: i]
            X_roll = X[factor_cols].iloc[i - roll_window: i]
            try:
                coefs = np.linalg.lstsq(
                    np.column_stack([np.ones(roll_window), X_roll.values]),
                    y_roll.values, rcond=None)[0]
                rolling_mom.append(coefs[factor_cols.index("MOM") + 1])
                rolling_dates.append(y.index[i])
            except Exception:
                rolling_mom.append(np.nan)
                rolling_dates.append(y.index[i])

        fig, ax = plt.subplots(figsize=(12, 4))
        ax.plot(rolling_dates, rolling_mom, color="#8e44ad", lw=1.2)
        ax.axhline(0, color="black", lw=0.8, ls="--")
        ax.fill_between(rolling_dates, rolling_mom, 0,
                        where=[v > 0 for v in rolling_mom],
                        alpha=0.2, color="#2ecc71", label="Positive MOM loading")
        ax.fill_between(rolling_dates, rolling_mom, 0,
                        where=[v <= 0 for v in rolling_mom],
                        alpha=0.2, color="#e74c3c", label="Negative MOM loading")
        ax.set_title("Rolling 1-Year Momentum (MOM) Beta",
                     fontweight="bold", fontsize=12)
        ax.set_ylabel("MOM β")
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)
        ...
        fig.autofmt_xdate()
        plt.tight_layout()
        path = os.path.join(save_dir, "ff5mom_rolling_mom_beta.png")
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close(fig)
        print(f"[PLOT] Saved → {path}")
    ...
    return {
        ...
        "t_alpha"   : t_alpha,
        "p_alpha"   : p_alpha,
        "mom_coef"  : mom_coef if "MOM" in factor_cols else np.nan,
        "mom_t"     : mom_t    if "MOM" in factor_cols else np.nan,
        "mom_p"     : mom_p    if "MOM" in factor_cols else np.nan,
        "r_squared" : r2,
...
def run_full_backtest(
    ...
    # -----------------------------------------------------------------------
    # D2) VANILLA OOS PROCESSING  (beta = 1)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # D3) VANILLA FREE-BETA OOS PROCESSING
    # -----------------------------------------------------------------------
    oos_van_fb_csv = os.path.join(OUTPUT_DIR, "oos_vanilla_freebeta_SP500.csv")
    if os.path.exists(oos_van_fb_csv) and not reload_oos:
        print(f"\n[OOS-VANILLA-FB] Loading cached from {oos_van_fb_csv}")
        oos_df_vanilla_fb = pd.read_csv(oos_van_fb_csv, parse_dates=["Date"])
    else:
        if "beta_hat" in pairs_df_johansen.columns:
            oos_df_vanilla_fb = run_oos_processing_vanilla_freebeta(
                pairs_df_johansen, prices)
        else:
            print("[OOS-VANILLA-FB] beta_hat not in pairs_df_johansen — "
                  "re-run with reload_pairs=True to enable free-beta.")
            oos_df_vanilla_fb = pd.DataFrame()
    ...
    # -----------------------------------------------------------------------
    # E2) RETURNS AGGREGATION — VANILLA β=1 STRATEGY
    # -----------------------------------------------------------------------
    print("[RETURNS] Aggregating daily returns (vanilla β=1 strategy) ...")
    ...
    if combined_df_vanilla.empty:
        ...
        van_ret_csv = os.path.join(OUTPUT_DIR, "combined_returns_vanilla_beta1.csv")
        combined_df_vanilla.to_csv(van_ret_csv)
        print(f"[RETURNS] Vanilla β=1 returns saved to {van_ret_csv}")

    # -----------------------------------------------------------------------
    # E3) RETURNS AGGREGATION — VANILLA FREE-β STRATEGY
    # -----------------------------------------------------------------------
    combined_df_vanilla_fb = pd.DataFrame()
    if not oos_df_vanilla_fb.empty:
        print("[RETURNS] Aggregating daily returns (vanilla free-β strategy) ...")
        combined_df_vanilla_fb = aggregate_returns_vanilla_freebeta(oos_df_vanilla_fb)
        if not combined_df_vanilla_fb.empty:
            fb_csv = os.path.join(OUTPUT_DIR, "combined_returns_vanilla_freebeta.csv")
            combined_df_vanilla_fb.to_csv(fb_csv)
            print(f"[RETURNS] Vanilla free-β returns saved to {fb_csv}")
    ...
    # -----------------------------------------------------------------------
    # F3) FAMA-FRENCH 5-FACTOR + MOMENTUM REGRESSION
    # -----------------------------------------------------------------------
    ff5mom_results = run_fama_french_momentum(combined_df, rates)
    ...
    # -----------------------------------------------------------------------
    # H4) VANILLA β=1 vs FREE-β PERFORMANCE COMPARISON
    # -----------------------------------------------------------------------
    if not combined_df_vanilla.empty and not combined_df_vanilla_fb.empty:
        _print_vanilla_beta_comparison_table(combined_df_vanilla,
                                             combined_df_vanilla_fb)
        _plot_vanilla_beta_comparison(combined_df_vanilla,
                                      combined_df_vanilla_fb)
    ...
    return {
        ...
        "oos_df_vanilla_fb"      : oos_df_vanilla_fb,
        "combined_df"            : combined_df,
        "combined_df_vanilla"    : combined_df_vanilla,
        "combined_df_vanilla_fb" : combined_df_vanilla_fb,
        "perf"                   : perf,
        "semi_perf"              : semi_perf,
